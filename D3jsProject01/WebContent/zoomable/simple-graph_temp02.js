...
SimpleGraph = function(elemid, options) {
  var self = this; // SimpleGraph 객체
  this.chart = document.getElementById(elemid); // id가 chart1인 div를 얻는다. 
  this.cx = this.chart.clientWidth;	// chart div의 넓이를 구한다.
  this.cy = this.chart.clientHeight; // chart div의 높이를 구한다.
  this.options = options || {}; // 옵션 파라미터 저장
  this.options.xmax = options.xmax || 30; // 옵션 xmax의 디폴트값은 30
  this.options.xmin = options.xmin || 0;  // 옵션 xmin의 디폴트값은 0
  this.options.ymax = options.ymax || 10; // 옵션 ymax의 디폴트값은 10
  this.options.ymin = options.ymin || 0;  // 옵션 ymin의 디폴트값은 0

  // 옵션 여부에 따라 padding을 다르게 세팅.
  this.padding = {
