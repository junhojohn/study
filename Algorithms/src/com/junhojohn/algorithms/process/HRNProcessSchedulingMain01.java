...
public class HRNProcessSchedulingMain01 {
	...
	public static void main(String[] args) {
		...
		try{
			...
			double[] hrnResultList = new double[numOfProcess];
			...
			for(int i = 0 ; i < numOfProcess ; i ++){
				...
				// calculate Highest Responsibility ratio
				// formula: (waitTime + executeTime) / executeTime
				hrnResultList[i] = ((double)turnTimeList[i] / (double)executeTimeList[i]);
			}
			...
			for(int i = 0 ; i < numOfProcess ; i ++){
				...
				System.out.println("Process" + processList[i] + "'s execute time:" + executeTimeList[i]);
				System.out.println("Process" + processList[i] + "'s wait time:" + waitTimeList[i]);
				System.out.println("Process" + processList[i] + "'s HRN value:" + hrnResultList[i]);
				System.out.println("============================");
			}
		}catch(Exception e){
			e.printStackTrace();
